...
import { type InputRef, Modal, message } from 'antd'
...
interface PersistPipelineResult {
  id: string
  name: string
  icon: string
}
...
interface StrategyDataPayload extends UpdateStrategyRequest {
  pipeline_id: string
  icon: string
}

function buildStrategyData(
  savedPipeline: PersistPipelineResult,
  editingRule: Partial<Strategy>,
  isEditingDefault: boolean
): StrategyDataPayload {
  const data: StrategyDataPayload = {
    name: editingRule.name?.trim(),
    pipeline_id: savedPipeline.id,
    icon: savedPipeline.icon
  }
  if (!isEditingDefault) {
    const conditions = editingRule.conditions_json
    data.conditions_json = typeof conditions === 'string' ? JSON.parse(conditions) : conditions
    data.logic = editingRule.logic
  }
  return data
}
...
async function persistStrategy(
  ...
  isCreating: boolean,
  editingRule: Partial<Strategy>,
  strategyData: StrategyDataPayload
): Promise<void> {
  if (isCreating) {
  ...
  if (editingId === undefined) {
    throw new Error('Cannot update strategy without an id')
  }
  await ragStrategyApi.update(editingId, strategyData)
}
...
const createPipelineFromRule = (rule: Partial<Strategy>): Pipeline => {
  ...
  // 3. 如果没有输入步骤，使用默认步骤
  const steps =
    normalizedSteps.length > 0 ? normalizedSteps : JSON.parse(JSON.stringify(DEFAULT_PIPELINE_STEP))
...
export function useRuleEditor({ rules, onSuccess }: UseRuleEditorOptions) {
  ...
  const lastInputRef = useRef<InputRef>(null)
  const editorRef = useRef<EditorRef>(null)
  const userInteractedRef = useRef(false)
  const initialRuleRef = useRef<Partial<Strategy>>({})

  // Is editing default rule
  const isEditingDefault = useMemo(() => editingRule.is_default === true, [editingRule])
  ...
  const handleAdd = useCallback(() => {
    const maxPriority = Math.max(0, ...rules.filter((r) => !r.is_default).map((r) => r.priority))
    const newRule = {
      ...JSON.parse(JSON.stringify(DEFAULT_RULE)),
  ...
  const handleNextStep = useCallback(() => {
    ...
    const allNames = rules.filter((r) => r.id !== editingRule.id).map((r) => r.name.trim())
  ...
  const handleSave = useCallback(
    async (pipeline: Pipeline) => {
      ...
      try {
        ...
        const savedPipeline = await persistPipeline(pipeline, isCreating, editingRule, uploadedIcon)
        const strategyData = buildStrategyData(savedPipeline, editingRule, isEditingDefault)
        await persistStrategy(savedPipeline, isCreating, editingRule, strategyData)
