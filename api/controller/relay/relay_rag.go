...
func HandleRAG(c *gin.Context, chatRequest *ChatRequest, ctx context.Context, messageStatus *MessageStatsInfo) ([]rag.SourceReference, error) {
	...
	if chatRequest.DatasetIsSoloFile() {
		logger.Infof(ctx, "【网络搜索】当前请求为单文件模式，跳过网络搜索分支：file_ids=%d，message_file_id=%d", len(chatRequest.FileIDs), chatRequest.MessageFileID)
...
func HandleKnowledgeSearchRag(c *gin.Context, queries []string, chatRequest *ChatRequest, ctx context.Context, messageStatus *MessageStatsInfo) ([]rag.SourceReference, error) {
	...
	// 设置知识范围和知识类型
	// KnowledgeScope 存储的是 hashID 字符串（逗号分隔）
	// KnowledgeType: 3=指定知识库
	if searchTarget.Type == "knowledge_base" && len(chatRequest.KnowledgeBaseIDs) > 0 {
		messageStatus.KnowledgeScope = strings.Join(chatRequest.KnowledgeBaseIDs, ",")
		messageStatus.KnowledgeType = model.KnowledgeTypeSpecificKB
	} else if searchTarget.Type == "file" && len(chatRequest.FileIDs) > 0 {
		messageStatus.KnowledgeScope = strings.Join(chatRequest.FileIDs, ",")
		messageStatus.KnowledgeType = model.KnowledgeTypeSpecificKB
	}
