...
func hasStreamResponseStarted(c *gin.Context) bool {
	...
	if collectorAny, exists := c.Get("stream_response_collector"); exists {
		if collector, ok := collectorAny.(*StreamResponseCollector); ok && collector != nil {
			content, reasoning := collector.GetContent()
...
func (w *LLMDeltaCollector) GetContent() (string, string, []relay_model.Tool) {
	...
	return w.content.String(), w.reasoningContent.String(), toolCalls
}
...
func runAgentLoop(c *gin.Context, requestCtx context.Context, agent *model.Agent, chatRequest *ChatRequest, messageStatus *MessageStatsInfo,
	...
	for turnCount < maxTurns {
				...
				if !isDuplicate {
					if !isAnsweringPhase {
						...
						if len(detectedToolCalls) > 0 {
							previewFileVersions := cloneToolFileVersions(sessionFileVersions)
							var validDetectedCalls []relay_model.Tool
							for _, toolCall := range detectedToolCalls {
								...
								if s, ok := argsStr.(string); ok {
									...
									argsString = string(bytes)
								}
								argsMap := map[string]interface{}{}
								_ = json.Unmarshal([]byte(argsString), &argsMap)
								key := buildToolDedupKey(getCurrentSkillName(), functionName, argsString, argsMap, previewFileVersions)

								if !executedTools[key] {
									validDetectedCalls = append(validDetectedCalls, toolCall)
									recordSessionFileMutation(previewFileVersions, functionName, argsMap)
								} else {
									logger.Warnf(ctx, "Ignoring duplicate tool call from content: %s", key)
								}
							}
		...
		if len(message.ToolCalls) > 0 {
			previewFileVersions := cloneToolFileVersions(sessionFileVersions)
			var newToolCalls []relay_model.Tool
			injectedTools := getInjectedToolNames()
			for _, toolCall := range message.ToolCalls {
				functionName := toolCall.Function.Name
				if _, exists := injectedTools[functionName]; !exists {
					logger.Warnf(ctx, "【技能运行】工具权限拦截: skill=%s, tool=%s, injected_tools_only", getCurrentSkillName(), functionName)
					continue
				}
			...
			for _, toolCall := range message.ToolCalls {
				...
				if err != nil {
					...
					if err != nil {
						...
						// alter the key for run_shell so a changed script can be re-run safely.
						key := buildToolDedupKey(getCurrentSkillName(), functionName, argsStrString, args, sessionFileVersions)
...
func sendStreamResponse(c *gin.Context, requestId, modelName, content, reasoningContent string, messageID int64, contentDeltas []string) {
	...
	// 5. Send DONE (in compact mode, let defer handle it to ensure process steps are sent first)
	if !config.IsSSECompactMode() {
