...
func (s *RecordingService) directFinalizeRecordingJob(ctx context.Context, job *model.RecordingJob, segments []model.RecordingJobSegment) (*model.File, error) {
	...
	data, err := loadRecordingArtifact(seg.StorageKey)
	if err != nil {
		return nil, fmt.Errorf("load segment %d failed: %w", seg.SegmentIndex, err)
	}

	transcodeStart := time.Now()
	transcodedContent, err := s.transcoder.Transcode(ctx, [][]byte{data}, job.TargetFormat)
	if err != nil {
