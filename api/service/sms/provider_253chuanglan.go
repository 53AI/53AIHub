...
func NewChuanglanProvider(account, password, signName string) SMSProvider {
...
// Send 发送短信验证码
// 模板: "{sign}您的验证码为：{vcode}，15分钟内有效，请勿汇露于他人！"
func (c *ChuanglanProvider) Send(mobile string, code string) error {
	// 构建消息内容
	message := fmt.Sprintf("【%s】您的验证码为：%s，15分钟内有效，请勿汇露于他人！", c.signName, code)
