...
func (s *ChunkerService) parseQAContentFromText(content string) []QA_pair {
	...
	normalized = regexp.MustCompile(`(?m)^(?:问题|问|Question|[Qq])\s*[:：]\s*`).ReplaceAllString(normalized, "问题：")
	normalized = regexp.MustCompile(`(?m)^(?:回答话术|回答|答案|答|Answer|[Aa])\s*[:：]\s*`).ReplaceAllString(normalized, "回答：")
