...
func (s *SearchService) SearchBatch(ctx context.Context, eid int64, reqs []*SearchRequest, userID *int64) ([]BatchSearchResult, error) {
	...
	collection, err := s.resolveBatchCollection(eid, normalized)
	if err != nil {
		logger.SysDebugf("【批量向量检索】解析批量 collection 失败，回退单条搜索: eid=%d, err=%v", eid, err)
		return s.searchBatchFallback(ctx, eid, normalized, userID)
	}
	...
	logger.SysDebugf("【批量向量检索】开始: eid=%d, query_count=%d, search_types=%v, queries=%v",
		eid, len(normalized), previewSearchTypesForDebug(normalized), previewQueriesForDebug(normalized, 8))
	...
	searchResponses, err := batchSearcher.SearchBatch(ctx, batchReq)
	if err != nil {
		logger.SysDebugf("【批量向量检索】Qdrant 批量搜索失败，回退单条搜索: eid=%d, collection=%s, err=%v", eid, collection, err)
		return s.searchBatchFallback(ctx, eid, normalized, userID)
	}
	batchTrace.add("vector_search_ms", time.Since(vectorSearchStart))
	if len(searchResponses) != len(normalized) {
		return nil, fmt.Errorf("批量搜索返回数量不匹配: expected=%d, got=%d", len(normalized), len(searchResponses))
	}
	...
	for i, searchResp := range searchResponses {
		...
		for j, result := range searchResp.Results {
			vectorResults[j] = vectorstore.SearchResult{
				ID:       result.ID,
				Score:    result.Score,
		...
		enriched, enrichErr := s.enrichVectorResults(eid, collection, vectorResults, normalized[i].Query, configService)
		batchTrace.add("enrich_ms", time.Since(enrichStart))
		if enrichErr != nil {
			results = append(results, BatchSearchResult{Query: normalized[i].Query, Error: enrichErr, StageTimings: batchTrace.snapshot()})
			continue
		}
		...
		if userID != nil && *userID != 0 {
			permissionStart := time.Now()
			enriched, err = s.filterResultsByFilePermission(eid, enriched, *userID)
			batchTrace.add("permission_ms", time.Since(permissionStart))
			if err != nil {
				results = append(results, BatchSearchResult{Query: normalized[i].Query, Error: fmt.Errorf("过滤文件权限失败: %v", err), StageTimings: batchTrace.snapshot()})
				continue
			}
		}
		...
		if len(enriched) > normalized[i].TopK {
			enriched = enriched[:normalized[i].TopK]
		}
		results = append(results, BatchSearchResult{Query: normalized[i].Query, Results: enriched, StageTimings: batchTrace.snapshot()})
	}

	logger.SysDebugf("【批量向量检索】完成: eid=%d, query_count=%d, collection=%s, mode=%s",
		eid, len(results), collection, batchSearchModeName(normalized))
	return results, nil
}
...
func (s *SearchService) canUseBatchVectorSearch(reqs []*SearchRequest) bool {
	...
	firstLibraryID := reqs[0].LibraryIDs[0]
	for _, req := range reqs[1:] {
		if len(req.LibraryIDs) != 1 || req.LibraryIDs[0] != firstLibraryID {
			return false
		}
	}
	return true
...
func (s *SearchService) batchSearchFallbackReason(reqs []*SearchRequest) string {
	...
	for i, req := range reqs {
		...
		if i > 0 && req.LibraryIDs[0] != reqs[0].LibraryIDs[0] {
			return fmt.Sprintf("request_%d_library_mismatch", i)
		}
	}
	return "unknown"
}
...
func (s *SearchService) vectorMatchEntities(eid int64, keywords []string) ([]model.Entity, error) {
	...
	for _, kw := range keywords {
		...
		queryVec64, err := s.embedding.GetQueryEmbedding(eid, kw, *config.EmbeddingChannelID, config)
		if err != nil {
			if firstErr == nil {
		...
		res, err := s.vectorDB.Search(context.Background(), req)
		if err != nil {
			if firstErr == nil {
			...
			if r.Metadata != nil {
				if v, ok := r.Metadata["entity_id"]; ok {
					...
					if _, ok := idSet[id]; !ok {
						...
						results = append(results, model.Entity{ID: id, Name: name})
						idSet[id] = struct{}{}
					}
				}
			}
...
func (s *SearchService) likeMatchEntitiesByKeywords(eid int64, keywords []string) []model.Entity {
	...
	for _, kw := range keywords {
		...
		entities, err := s.likeMatchEntities(eid, kw, 10)
		if err != nil {
			logger.SysErrorf("【缩小实体范围】【明显告警】LIKE回退查询失败: eid=%d, keyword=%s, err=%v", eid, kw, err)
			continue
		}
...
func (s *SearchService) likeMatchEntitiesByKeywordsScoped(eid int64, keywords []string, libraryIDs []int64, fileIDs []int64) []model.Entity {
	...
	for _, kw := range keywords {
		...
		entities, err := s.likeMatchEntitiesScoped(eid, kw, 10, libraryIDs, fileIDs)
		if err != nil {
			logger.SysErrorf("【缩小实体范围】【明显告警】LIKE范围查询失败: eid=%d, keyword=%s, err=%v", eid, kw, err)
			continue
		}
		for _, entity := range entities {
			if entity.ID <= 0 {
...
func (s *SearchService) singleVectorSearch(eid int64, req *SearchRequest, configService *ChunkConfigService) ([]SearchResultItem, error) {
	...
	fileScopedSearch := len(req.FileIDs) > 0

	// 处理知识库ID（文件级检索时跳过，避免扫描与 file_ids 无关的库）
	if !fileScopedSearch {
		libraryMap, err := s.batchGetLibrariesByIDs(eid, req.LibraryIDs)
		if err != nil {
			logger.SysLogf("批量获取库信息失败: %v", err)
	...
	// 文件ID 不再参与检索过滤，只用于结果阶段的权限过滤。
	// 当请求没有显式库范围时，兼容性兜底：如果传了文件ID，则先推导出所属库，再按库级检索。
	if len(req.LibraryIDs) == 0 && len(req.FileIDs) > 0 {
		files, err := model.GetFilesByIDs(eid, req.FileIDs)
		if err != nil {
		...
		for _, file := range files {
			if _, exists := libraryIDSet[file.LibraryID]; exists {
				continue
			}
			libraryIDSet[file.LibraryID] = struct{}{}
...
func (s *SearchService) enrichVectorResultBatches(eid int64, batches []vectorResultBatch, query string, configService *ChunkConfigService) ([]SearchResultItem, error) {
	...
	for _, vectorResult := range filteredVectorResults {
		chunk := chunkMap[vectorResult.ID]
	...
	for _, vectorResult := range filteredVectorResults {
		...
		if docChunk != nil {
			// 优先使用文档级配置
			chunkConfig, configErr = configService.GetConfig(eid, &docChunk.LibraryID, model.ChunkTypeDefault)
		} else {
			// 回退到检索块的库级配置
			chunkConfig, configErr = configService.GetConfig(eid, &rc.LibraryID, model.ChunkTypeDefault)
		}
		...
		if configErr == nil && chunkConfig != nil {
			...
			if chunkConfig.KnowledgeIncludeFileName {
				fileInfo, fileExists := fileInfoMap[rc.FileID]
...
func (s *SearchService) batchGetLibrariesByIDs(eid int64, libraryIDs []int64) (map[int64]*model.Library, error) {
	...
	var libraries []model.Library
	if err := s.db.Where("eid = ? AND id IN ?", eid, uniqueLibraryIDs).Find(&libraries).Error; err != nil {
		return nil, err
	}

	result := make(map[int64]*model.Library, len(libraries))
	for i := range libraries {
		result[libraries[i].ID] = &libraries[i]
	}
	return result, nil
}
