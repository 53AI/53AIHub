...
// SandboxOutputSnapshot stores the content hash of files under output/.
...
// SnapshotSandboxOutputFiles captures the current output/ subtree.
func SnapshotSandboxOutputFiles(root string) (*SandboxOutputSnapshot, error) {
	snapshot, _, err := captureSandboxOutputFiles(nil, root)
	return snapshot, err
}

// DiffSandboxOutputFiles returns only new or changed output/ files.
...
// Call this before the first tool mutation in a conversation so pre-existing output/ files
